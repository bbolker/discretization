...
- need to choose *appropriate* model complexity
   - model size (selection methods) or complexity penalty (shrinkage methods)
   - estimate out-of-sample error without (explicit) cross-validation (AIC, Cp, BIC, out-of-bag error, ...)
   - or explicit cross-validation
- tune model **hyperparameters** (e.g. via cross-validation)
...
- focused on high dimensions, depends on strong assumptions
    - sparsity (e.g. $s \approx n/\log p$) @belloni_inference_2013
...
- thus we need $s \approx  12 - 30$ (or less!)
...
- **approximate sparsity**/**compressibility** (???):  $|\beta(k)| \leq C^k-r$, $r>0$ 
